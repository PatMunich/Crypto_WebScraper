...
class TelegramManager:
    ...
    def formatMessage(self, in_message):  # NOQA
        ...
        if self.first_run:
            ...
            formatted_message = f"> daily subnet rewards < \n" \
                                f"▪ {keys[0]} : {in_message[keys[0]]} [+{round(in_message[keys[0]] - self.last_message[keys[0]], 2)}]\n" \
                                f"▪ {keys[1]} : {in_message[keys[1]]} [+{round(in_message[keys[1]] - self.last_message[keys[1]], 2)}]\n" \
                                f"▪ {keys[2]} : {in_message[keys[2]]} [+{round(in_message[keys[2]] - self.last_message[keys[2]], 2)}]\n" \
                                f"▪ {keys[3]} : {in_message[keys[3]]} [+{round(in_message[keys[3]] - self.last_message[keys[3]], 2)}]\n"
